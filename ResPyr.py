...
class PyramidFeatures(nn.Module):
    def __init__(self, C2, C3, C4, C5, feature_size=256):
        super(PyramidFeatures, self).__init__()

        self.P5_1 = nn.Conv2d(C5, feature_size, kernel_size=1, stride=1, padding=0)
        self.P5_upsampled = nn.Upsample(scale_factor=2, mode='nearest')
        self.P5_2 = nn.Conv2d(feature_size, feature_size, kernel_size=3, stride=1, padding=1)

        self.P4_1 = nn.Conv2d(C4, feature_size, kernel_size=1, stride=1, padding=0)
        self.P4_upsampled = nn.Upsample(scale_factor=2, mode='nearest')
        self.P4_2 = nn.Conv2d(feature_size, feature_size, kernel_size=3, stride=1, padding=1)

        self.P3_1 = nn.Conv2d(C3, feature_size, kernel_size=1, stride=1, padding=0)
        self.P3_upsampled = nn.Upsample(scale_factor=2, mode='nearest')
        self.P3_2 = nn.Conv2d(feature_size, feature_size, kernel_size=3, stride=1, padding=1)

        self.P2_1 = nn.Conv2d(C2, feature_size, kernel_size=1, stride=1, padding=0)
...
# %%
class ResPyr(nn.Module):
    def __init__(self, num_classes, block=BasicBlock, layers=[2, 2, 2, 2]):
        super(ResPyr, self).__init__()
        self.inplanes = 64
        ...
        self.fpn = PyramidFeatures(64, 128, 256, 512)
        ...
        self.final = nn.Conv2d(256, num_classes, kernel_size=3, stride=1, padding=1)
    ...
    def forward(self, x):
        ...
        feature = self.fpn([x1, x2, x3, x4])

        feature = self.smooth(feature)
        feature = self.smooth2(feature)

        output = self.final(feature)

        return F.interpolate(output, scale_factor=4, mode='bilinear', align_corners=True)

# %%
# Testing Respyr Model
model = ResPyr(num_classes=1)  # binary segmentation
input_tensor = torch.randn(1, 3, 256, 256)  # RGB ultrasound image
...
class UltrasoundDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        mask_path = os.path.join(self.data_dir, img_name.replace('.png', '_mask.png'))
...
def train_model(model, train_loader, val_loader, criterion, optimizer, num_epochs, early_stopping_patience=30):
    train_losses = []
    train_accuracies = []
...
def visualize_predictions(model, dataloader, num_images=20, save_path = 'ResPyr_CVC_Clinic.png'):
    model.eval()
    lesion_cases = []
...
file_dir = '/home/rizk_lab/shared/Sony/CVC_ClinicDB'
# Main execution
...
visualize_predictions(model, test_loader, num_images=20, save_path='ResPyr_CVC_Clinic.png')